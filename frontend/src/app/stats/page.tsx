...
export default async function StatsPage({ searchParams }: PageProps) {
    ...
    if (!nationalStats || nationalStats.total === 0) {
      return (
        <div className="space-y-6">
          <div>
            <h1 className="text-2xl font-bold text-chalk mb-1">{t("stats.intl.title")}</h1>
          </div>
          <Suspense>
            <FilterBar
            activeLeague={league}
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {ns.by_confidence.map((c) => (
                <div
                  key={c.confidence}
                  className="rounded-xl border border-line bg-ink-700/60 p-4 space-y-2"
                >
                  <p className="text-sm font-semibold capitalize text-chalk-2">
                    {c.confidence === "HIGH" && "🟢 "}
                    {c.confidence === "MEDIUM" && "🟡 "}
                    {c.confidence === "LOW" && "🔴 "}
                    {c.confidence.toLowerCase()}
                    <span className="ml-2 text-xs font-normal text-chalk-3">
                  ...
                  <AccuracyBar
                    ...
                    color={
                      c.confidence === "HIGH"   ? "bg-win" :
                      c.confidence === "MEDIUM" ? "bg-est" :
                      "bg-chalk-2"
                    }
                  />
  ...
  return (
    <div className="space-y-10">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-chalk mb-1">{t("stats.title")}</h1>
        <p className="text-sm text-chalk-3">
        ...
        <div className="rounded-xl border border-est/40 bg-est/10 p-4 text-sm">
          ...
          {s.methodology.regimes && s.methodology.regimes.length > 0 && (
            <div className="mt-3 space-y-1">
              <div className="grid grid-cols-[1fr_4.5rem_4.5rem_3.5rem] gap-2 text-[11px] text-chalk-3 uppercase tracking-wide">
              ...
              {s.methodology.regimes.map((r) => (
                ...
                >
                  ...
                  <span className="text-right tabular-nums text-chalk-3">{r.stats.total}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {s.injury_adjustment && s.injury_adjustment.matches >= 5 && (
        <div className="rounded-xl border border-line bg-ink-700/60 p-4 text-sm">
          <p className="font-semibold text-chalk-2 mb-1">{t("stats.injury.title")}</p>
          <p className="text-chalk-2 text-xs mb-2">
            {t("stats.injury.body", { n: s.injury_adjustment.matches })}
          </p>
          <div className="flex gap-6 tabular-nums">
            <span>
              1×2: raw {(s.injury_adjustment.raw_result_accuracy * 100).toFixed(1)}% →{" "}
              <span className={s.injury_adjustment.adj_result_accuracy >= s.injury_adjustment.raw_result_accuracy ? "text-win" : "text-lose"}>
                adj {(s.injury_adjustment.adj_result_accuracy * 100).toFixed(1)}%
              </span>
            </span>
            <span>
              O/U: raw {(s.injury_adjustment.raw_goals_accuracy * 100).toFixed(1)}% →{" "}
              <span className={s.injury_adjustment.adj_goals_accuracy >= s.injury_adjustment.raw_goals_accuracy ? "text-win" : "text-lose"}>
      ...
      {/* ── Top AI Picks ──────────────────────────────────────────────────── */}
      {topPicks && (
        <section className="rounded-xl border border-est/40 bg-est/10 p-5">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-lg">⚡</span>
            <h2 className="text-sm font-semibold text-est uppercase tracking-wide">
              {t("stats.topPicks.title")}
            </h2>
            <span className="ml-auto text-xs text-chalk-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            <div className="rounded-lg bg-ink-700/80 border border-est/40 p-4 text-center">
              <p className="text-xs text-chalk-3 uppercase tracking-wide mb-1">{t("stats.topPicks.accuracy")}</p>
              <p className={`text-3xl font-bold ${
                topPicks.accuracy >= 0.57 ? "text-win" :
                topPicks.accuracy >= 0.48 ? "text-est" : "text-lose"
              }`}>
            ...
            <div className="rounded-lg bg-ink-700/80 border border-est/40 p-4 text-center">
              <p className="text-xs text-chalk-3 uppercase tracking-wide mb-1">{t("stats.topPicks.avgProb")}</p>
              <p className="text-3xl font-bold text-est">
                {Math.round(topPicks.avg_pick_prob * 100)}%
              </p>
              <p className="text-xs text-chalk-3 mt-1">{t("stats.topPicks.topPickConfidence")}</p>
            </div>

            <div className="rounded-lg bg-ink-700/80 border border-est/40 p-4 text-center">
              <p className="text-xs text-chalk-3 uppercase tracking-wide mb-1">{t("stats.topPicks.totalPicks")}</p>
              <p className="text-3xl font-bold text-chalk">{topPicks.total}</p>
              <p className="text-xs text-chalk-3 mt-1">{t("stats.topPicks.perMatchDay")}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {topPicks.result_picks > 0 && (
              <div className="rounded-lg bg-ink-700/60 border border-line p-3">
                <p className="text-xs font-semibold text-chalk-2 uppercase mb-2">{t("stats.topPicks.result")}</p>
                <div className="flex items-end justify-between">
                  <div>
                    <p className={`text-2xl font-bold ${
                      topPicks.result_accuracy >= 0.57 ? "text-win" :
                      topPicks.result_accuracy >= 0.48 ? "text-est" : "text-lose"
                    }`}>{pct(topPicks.result_accuracy)}</p>
                    <p className="text-xs text-chalk-3">{t("stats.topPicks.correctN", { c: topPicks.result_correct, t: topPicks.result_picks })}</p>
                  </div>
                  <p className="text-xs text-chalk-3">{t("stats.topPicks.picks", { n: topPicks.result_picks })}</p>
                </div>
              </div>
            )}
            {topPicks.goals_picks > 0 && (
              <div className="rounded-lg bg-ink-700/60 border border-line p-3">
                <p className="text-xs font-semibold text-chalk-2 uppercase mb-2">{t("stats.topPicks.ou")}</p>
                <div className="flex items-end justify-between">
                  <div>
                    <p className={`text-2xl font-bold ${
                      topPicks.goals_accuracy >= 0.57 ? "text-win" :
                      topPicks.goals_accuracy >= 0.48 ? "text-est" : "text-lose"
                    }`}>{pct(topPicks.goals_accuracy)}</p>
                    <p className="text-xs text-chalk-3">{t("stats.topPicks.correctN", { c: topPicks.goals_correct, t: topPicks.goals_picks })}</p>
                  </div>
                  <p className="text-xs text-chalk-3">{t("stats.topPicks.picks", { n: topPicks.goals_picks })}</p>
                </div>
              </div>
            )}
          </div>

          <p className="text-[10px] text-chalk-3 mt-3 leading-relaxed">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Last 7 days */}
          <div className="rounded-xl border border-line bg-ink-700/60 p-4 space-y-3">
            <p className="text-sm font-semibold text-chalk-2">
              {t("stats.last7")}
              <span className="ml-2 text-xs font-normal text-chalk-3">
                {t("stats.nMatches", { n: last7.total })}
              </span>
            </p>
            <AccuracyBar label={t("stats.result")} value={last7.result_accuracy} color="bg-win" />
            <AccuracyBar label={t("stats.ou")}     value={last7.goals_accuracy}  color="bg-chalk-2" />
            <AccuracyBar label={t("stats.both")}   value={last7.both_accuracy}   color="bg-chalk-2" />
          </div>
          ...
          <div className="rounded-xl border border-line bg-ink-700/60 p-4 space-y-3">
            ...
            <AccuracyBar label={t("stats.result")} value={last30.result_accuracy} color="bg-win" />
            <AccuracyBar label={t("stats.ou")}     value={last30.goals_accuracy}  color="bg-chalk-2" />
            <AccuracyBar label={t("stats.both")}   value={last30.both_accuracy}   color="bg-chalk-2" />
          </div>
        </div>
      </section>
      ...
      <section>
        <h2 className="text-sm font-semibold text-chalk-2 uppercase tracking-wide mb-3">
          {t("stats.byConfidence")} <span className="text-chalk-3 normal-case">{t("stats.byConfidenceClub")}</span>
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        {s.by_confidence_national && s.by_confidence_national.some((c) => c.total > 0) && (
          <div className="mt-4">
            <h3 className="text-xs font-semibold text-chalk-3 uppercase tracking-wide mb-2">
              {t("stats.internationals")} <span className="text-chalk-3 normal-case">{t("stats.intlScale")}</span>
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {s.by_confidence_national.filter((c) => c.total > 0).map((c) => (
                <div
                  key={`nat-${c.confidence}`}
                  className="rounded-xl border border-line bg-ink-700/40 p-4 space-y-2"
                >
                  <p className="text-sm font-semibold capitalize text-chalk-2">
                    {c.confidence === "high" && "🟢 "}
                    {c.confidence === "medium" && "🟡 "}
                    {c.confidence === "low" && "🔴 "}
                    {c.confidence}
                    <span className="ml-2 text-xs font-normal text-chalk-3">{t("stats.nMatches", { n: c.total })}</span>
                  </p>
                  <AccuracyBar
                    label={t("stats.resultAccuracyBar")}
                    value={c.result_accuracy}
                  ...
                  />
                  <p className="text-xs text-chalk-3">{t("stats.correctFrac", { c: c.result_correct, t: c.total })}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Result outcomes */}
          <div className="rounded-xl border border-line bg-ink-700/60 p-4 space-y-3">
            <p className="text-sm font-semibold text-chalk-2">{t("stats.matchResult")}</p>
            {resultOutcomes.map((o) => (
              <div key={o.predicted} className="space-y-1">
                <AccuracyBar
                  ...
                  value={o.accuracy}
                  color={
                    o.predicted === "H" ? "bg-win" :
                    o.predicted === "D" ? "bg-chalk-2" :
                    "bg-chalk-2"
                  }
                />
                <p className="text-xs text-chalk-3 pl-1">
          ...
          {/* O/U outcomes */}
          <div className="rounded-xl border border-line bg-ink-700/60 p-4 space-y-3">
            <p className="text-sm font-semibold text-chalk-2">{t("stats.overUnder")}</p>
            {goalsOutcomes.map((o) => (
              <div key={o.predicted} className="space-y-1">
                <AccuracyBar
                  label={o.predicted === "OVER" ? t("stats.over25") : t("stats.under25")}
                  value={o.accuracy}
                  color={o.predicted === "OVER" ? "bg-est" : "bg-chalk-2"}
                />
                <p className="text-xs text-chalk-3 pl-1">
                  {t("stats.correctFrac", { c: o.correct, t: o.total })}
                </p>
      ...
      {btts && (
        <section>
          ...
          {/* What is GG/NG */}
          <div className="rounded-xl border border-line bg-ink-700/40 p-3 mb-3 text-xs text-chalk-2 leading-relaxed">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3">
            <div className="rounded-xl border border-line bg-ink-700/60 p-4 text-center">
              <p className="text-xs text-chalk-3 uppercase tracking-wide mb-1">{t("stats.bttsSample")}</p>
              <p className="text-2xl font-bold text-chalk">{btts.total_gg + btts.total_ng}</p>
              <p className="text-xs text-chalk-3 mt-1">{t("stats.bttsCompletedWithLambda")}</p>
              <div className="flex justify-center gap-4 mt-2 text-xs">
                <span className="text-win font-semibold">{btts.total_gg} GG</span>
                <span className="text-chalk-3">/</span>
                <span className="text-lose font-semibold">{btts.total_ng} NG</span>
              </div>
            </div>
            <div className="rounded-xl border border-line bg-ink-700/60 p-4 text-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="rounded-xl border border-line bg-ink-700/60 p-4">
              <p className="text-xs text-chalk-3 mb-1">{t("stats.bttsGGPredictions")}</p>
              <p className="text-xl font-bold text-chalk">{btts.predicted_gg}</p>
              <p className="text-xs text-chalk-3 mt-1 leading-tight">{t("stats.bttsGGPredictedSub")}</p>
            </div>
            <div className="rounded-xl border border-line bg-ink-700/60 p-4">
              <p className="text-xs text-chalk-3 mb-1">{t("stats.bttsNGPredictions")}</p>
              <p className="text-xl font-bold text-chalk">{btts.predicted_ng}</p>
              <p className="text-xs text-chalk-3 mt-1 leading-tight">{t("stats.bttsNGPredictedSub")}</p>
            </div>
          </div>
        </section>
      )}
