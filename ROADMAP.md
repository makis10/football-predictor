...
| 3 | ~~**Ισοπαλίες**~~ — **κλειστό 2026-09-04**: oracle AUC 0,5646, δικό μας 0,5649. Είμαστε ΠΑΝΩ στο ταβάνι· η πιθανότητα ισοπαλίας δεν μεταβάλλεται αρκετά μεταξύ αγώνων (sd 0,045 vs 0,163 του γηπεδούχου). Δοκιμάστηκε walk-forward Dixon-Coles: ως features μηδέν, ως blend το cal διαλέγει w=0 | ⛔ Κλειστό |
| 4 | ~~**Over 2.5**~~ — **δοκιμάστηκε 2026-09-04**: ablation (μόνο το league one-hot μετράει, το xG βλάπτει), walk-forward league/team rates (null σε 4 σεζόν, P=0,651), Dixon-Coles (null), MLP 0,5695 και CatBoost με team categoricals 0,5726 έναντι 0,5753 του XGBoost. Το περιθώριο δεν είναι σε μοντέλο — είναι σε δεδομένα (συνθέσεις, τραυματισμοί ως features) | ⛔ Κλειστό ως modelling |
| 5 | ~~**Cross-fitted calibration**~~ — **απορρίφθηκε 2026-09-04**: χάνει (1,0144 vs 1,0132, drawAUC 0,5397 vs 0,5464). Η νίκη της 3/9 ήταν με isotonic· το matrix scaling κορένεται πολύ νωρίτερα | ⛔ Κλειστό |
| 6 | **Τραυματισμοί ως features** — δεδομένα υπάρχουν από ~2020/21 (~115 requests). Σήμα υπαρκτό στο 1×2 (p=0,034) αλλά ~0,0006 log-loss· **μηδέν στα γκολ** (p=0,785). Ταίριασμα ονομάτων 74,9%. Η σταθμισμένη εκδοχή (ποιος λείπει, όχι πόσοι) είναι αμέτρητη ακόμα | 🟡 Ανοιχτό, χαμηλή προτεραιότητα |
| 7 | **BTTS κατώφλι** — 0,52 με macro-F1 δίνει ισορροπία GG/NG· 0,50 δίνει +2,1 μονάδες ακρίβεια αλλά καταρρέει το NG recall στο 6%. Προϊοντική απόφαση | 🟡 Ανοιχτό |
...
## 🔴 Phase 9 — Ό,τι δείχνει η σελίδα πρέπει να στέκει (2026-09-07)

Follow-up audit του Phase 8 με 13 πράκτορες (έξι αντίπαλοι επαληθευτές, έξι
διαγνωστικοί, ένας σύνθεσης). **Οι μισές «ανησυχητικές» διαπιστώσεις δεν
επέζησαν** και καταγράφονται διορθωμένες στο CHANGELOG. Ό,τι επέζησε δεν ήταν
αποτυχία μοντέλου — ήταν νούμερα που η σελίδα έδειχνε χωρίς να τα στηρίζει.

### ✅ 9.1 · Τέσσερις αγώνες έλεγαν 100% — και ένα 0%

`over_2_5_prob = 1.0` σε τέσσερα ζωντανά fixtures με confidence `high`. Το
μοντέλο δεν το πίστευε ποτέ (raw 0,78–0,81). Τρεις κρίκοι: isotonic σε δυαδικές
ετικέτες βάζει το τερματικό μπλοκ στο k/k· η προβολή συνέπειας
**απενεργοποιείται ακριβώς πάνω στις αδύνατες τιμές** (`if proj:` κρατά αυτό που
έσπασε το fit)· κανένα write path δεν έκανε clamp. Νέο `prob_bounds.py` με
`SmoothedIsotonic` (pseudo-count shrinkage ανά μπλοκ) + `FIT_EPS` + serving clamp
μέσα στο `project_probs_coherent`, όπου συγκλίνουν και τα τρία μονοπάτια.

Walk-forward σε 1.460 κριθέντες: plain isotonic 0,6806 (**χειρότερο από το να μην
βαθμονομείς**), smoothed 0,6722. P(better)=0,903 — κάτω από τον πήχη 0,95, οπότε
δεν διεκδικείται ως νίκη επίδοσης· το επιχείρημα είναι ότι το plain σέρβιρε
1,0000 για πεποίθηση 0,78.

### ✅ 9.2 · Αγκύρωση Over/Under & BTTS στο w=0.85

Τα δύο μοναδικά headline probabilities χωρίς περιεχόμενο αγοράς, και τα δύο
μοναδικά που μετρούν στο κέρμα (AUC 0,5222 και 0,5020). Καμία εσωτερική βέλτιστη
τιμή στο sweep — μονότονη πτώση ως w=1,00. Κάλυψη 98,4% / 94,4% των ήδη
τιμολογημένων fixtures. Νέα στήλη `raw_btts_prob` (0034) ώστε το EV gate να μη
συγκρίνει την αγορά με τον εαυτό της· `under_odds` (0035) στο `odds_history`,
δωρεάν.

### ✅ 9.3 · Τα δελτία τύπωναν +64% EV

`combined_prob × total_odds − 1` = +64,4% μέσο (+192% longshot) σε προϊόν που
αποδίδει −23,43%. Πλέον `combined_prob` = γινόμενο de-vigged τιμών αγοράς, άρα
`overround^-L` εξ ορισμού· το δικό μας γινόμενο αποθηκεύεται ως `model_prob` και
δείχνεται δίπλα. Το longshot **δεν** κόπηκε: η ανακατασκευή της 2026-09-01 το
είχε ήδη διορθώσει (21 σκέλη, 52,4% πραγματικό έναντι 54,8% τιμής).

### ✅ 9.4 · Τιμιότητα της σελίδας στατιστικών

- Κείμενο «43% μοντέλο / 57% αγορά» ενώ το βάρος ήταν 0,85 — **λάθος κατά 28
  μονάδες**. Το test που έπρεπε να το πιάσει καρφίτσωνε το literal `"57%"`·
  τώρα παράγει και τα δύο ποσοστά από τη σταθερά, σε δύο γλώσσες.
- Τα χρώματα αντέστρεφαν την αλήθεια (πράσινο στο 0,57 ≈ base rate του OVER).
  Κάθε ποσοστό κουβαλά πλέον τη δική του βάση και το accent ακολουθεί τη διαφορά.
- Διαγράμματα βαθμονόμησης: AUC + Brier resolution, γιατί ένα βαθμονομημένο
  σταθερό ζωγραφίζει τέλεια διαγώνιο. Το υπόμνημα «bubble size = sample count»
  ήταν ψευδές για 5 από 6 σημεία.
- Εθνικές: `national_total` σε κάθε slice (μια γραμμή regime ήταν 79/80 εθνικές)·
  date filter στο `/national/stats` (2.632 → 208, τα 2.424 ήταν backfilled replays).
- «Top AI Picks +18%» σύγκρινε hit rate μιας έκβασης με three-way accuracy.
  Τώρα: +1,1pp έναντι όσων δήλωσαν οι ίδιες οι επιλογές.
- `btts_prediction`: γραφόταν από τρία μονοπάτια, διαβαζόταν από κανένα. Πλέον
  σερβίρεται και οι δύο επιφάνειες το χρησιμοποιούν αντί για hardcoded 0,5.

### 🔜 9.5 · Τι μένει

- **Ταβάνι BTTS**: το oracle test λέει 0,5704 και η αγορά πιάνει 0,5706 — είναι
  στο ταβάνι της. Η πληροφορία είναι **εξ ολοκλήρου supremacy + total**: BTTS
  παραγόμενο από 1×2+O/U της αγοράς (0,5876) ξεπερνά την ίδια τη γραμμή BTTS
  (0,5824). Άρα το headroom αγοράζεται δωρεάν από τιμές που ήδη έχουμε — **δεν
  χτίζεται με καλύτερο classifier**.
- **Δελτία**: δομικά πληρώνουν γκανιότα. Break-even θέλει +4,60 μονάδες αληθινό
  edge ανά σκέλος· το μετρημένο είναι −2,20. Καμία ρύθμιση δεν αλλάζει πρόσημο —
  μόνο η τιμιότητα της παρουσίασης ήταν στο πεδίο.
- **Μην ξανακοιταχτούν**: παράθυρα 258/139 γραμμών (12 και 3 αγωνιστικές
  ημέρες), sweep βάρους αγκύρωσης in-sample σε τέσσερις εβδομάδες, το 0/19 του
  longshot (P=0,196 με τις δικές του πιθανότητες).
